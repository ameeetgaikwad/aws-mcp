import { SSHConnectionConfig } from "../services/SSHService";

export const defaultSSHConfig: SSHConnectionConfig = {
  host: "44.201.255.110",
  port: 22,
  username: 'ubuntu',
  privateKey: `-----BEGIN RSA PRIVATE KEY-----

lv5q4T2ewsnvHwZSxsTocqYRVUL9mC6YBePfZFLyt64EGF9isowL
-----END RSA PRIVATE KEY-----`,
};

/**
 * Get SSH config with optional overrides
 */
export function getSSHConfig(overrides: Partial<SSHConnectionConfig> = {}): SSHConnectionConfig {
  return {
    ...defaultSSHConfig,
    ...overrides
  };
} 