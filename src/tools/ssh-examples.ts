...
export const installPackage = async (packageName: string): Promise<void> => {
  ...
  try {
    ...
    if (result.code !== 0) {
      throw new Error(`Package installation failed with exit code ${result.code}`);
    }
 ...
 */
export const deployApplication = async (sourcePath: string, destPath: string): Promise<void> => {
...
export const getSystemInfo = async (): Promise<string> => {
  ...
  try {
    ...
    if (result.code !== 0) {
      throw new Error(`Failed to get system information with exit code ${result.code}`);
    }
...
export const executeCustomCommand = async (
  ...
  const sshService = new SSHService(getSSHConfig(configOverrides));
